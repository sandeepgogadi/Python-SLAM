...
from robot import *
from robot_library import filter_step
from robot_04_a_project_landmarks import compute_scanner_cylinders, write_cylinders
...
def find_cylinder_pairs(cylinders, reference_cylinders, max_radius):
    ...
    for i, c in enumerate(cylinders):
        x, y = c
        for j, rc in enumerate(reference_cylinders):
            rx, ry = rc
            dist = (rx - x)**2 + (ry - y)**2
...
def estimate_transform(left_list, right_list, fix_scale = False):
    ...
    for l, r in zip(left_list, right_list):
        lx, ly = l[0] - lc[0], l[1] - lc[1]
        rx, ry = r[0] - rc[0], r[1] - rc[1]
...
if __name__ == '__main__':
    ...
    for i in range(len(logfile.scan_data)):
        ...
        world_cylinders = [RobotLogfile.scanner_to_world(pose, c)
