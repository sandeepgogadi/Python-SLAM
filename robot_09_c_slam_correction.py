...
from numpy import *
from robot_library import get_observations_ekfslam, write_cylinders,\
...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        self.state = append(new_state, self.state[3:,])
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        rows, cols = shape(self.covariance)
        C = zeros((rows+2, cols+2))
        C[0:rows, 0:cols] = self.covariance
        C[rows:, cols:] = diag([1e10, 1e10])
        self.covariance = C
    ...
    def correct(self, measurement, landmark_index):
        ...
        H = zeros((2, size))
        H[0:2, 0:3] = H3
        H[0:2, 3+2*landmark_index: 3+2*landmark_index+2] = H3[0:2, 0:2]*-1
        ...
        self.covariance = dot(eye(shape(self.state)[0]) - dot(K, H), self.covariance)
...
if __name__ == '__main__':
    ...
    for i in range(len(logfile.motor_ticks)):
        ...
        # Correction.
        observations = get_observations_ekfslam(
            logfile.scan_data[i],
